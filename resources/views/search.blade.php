            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form Method = 'POST' class="space-y-4 md:space-y-6" action="{{route('search.result')}}">
                    ...
                    <div>
                        ...
                        @error('code')
                            <p class = "text-white my-2 rounded-lg text-lg text-center p-2" style="background-color: #FF6B6B">{{ $message }} </p>
                        @enderror
                        @if (session('error'))
                        <p class = "text-white my-2 rounded-lg text-lg text-center p-2" style="background-color: #FF6B6B">{{ session('error') }} </p>
                        @endif
                    </div>
